...
namespace YeZiFang.App.Service
{
    /// <summary>
    /// Mongodb服务类
    /// </summary>
    public class MongoService : IMongoService
    {
        ...
        /// <returns></returns>
        public PagingList<T> PageList<T>(Hashtable hs, int pageIndex, int pageSize, string sortFiled = "", bool isAsc = false)
        {
            IMongoCollection<T> collection = MongoDatabase.GetCollection<T>(GetCollectionName(typeof(T)));
            var builder = Builders<T>.Filter;
            var filter = builder.Empty;
            ...
            result.TotalCount = collection.CountDocuments(filter);
        ...
        /// <param name="isAsc">是否升序</param>
        /// <returns></returns>
        public PagingList<T> PageList<T>(int pageIndex, int pageSize, string sortFiled = "", bool isAsc = false)
        {
            IMongoCollection<T> collection = MongoDatabase.GetCollection<T>(GetCollectionName(typeof(T)));
            var builder = Builders<T>.Filter;
            var filter = builder.Empty;
